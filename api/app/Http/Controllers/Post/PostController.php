...
class PostController extends Controller
{
    ...
    public function saveShortVideo(Request $request)
    {

        $validator = Validator::make($request->all(), [
            'files' => 'required|file|mimes:mp4|max:10240', // Validate file: required, MP4 format, and maximum 10MB (10240 KB)
        ]);
        if ($validator->fails()) {
            return response()->json(['errors' => $validator->errors()], 422);
        }
        ...
        $data['status'] = 1;
        $resdata['shortvideoId'] = ShortVideos::insertGetId($data);
        return response()->json($resdata);
    }
    ...
    public function allShortVideoPost(Request $request)
    {
        ...
        // Initialize the query for ShortVideos
        $query = ShortVideos::orderBy('id', 'desc'); 

        // Apply the status filter if it is not null
        if ($searchQuery !== null) {
        ...
        $modifiedCollection = $paginator->getCollection()->map(function ($item) {
            return [
                'id'            => $item->id,
                'files'         => !empty($item->files) ? url($item->files) : "",
                'status'        => $item->status,
            ];
        });
    ...
    public function deleteShorVidoes(Request $request)
    {


        $id     = $request->id;
        $status = $request->status;
        $row = ShortVideos::where('id', $id)->first();
        if ($row) {
